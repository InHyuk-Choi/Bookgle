...
def list_create_pheeds(request):
    if request.method == 'GET':
        ...
        page = paginator.paginate_queryset(pheeds, request)
        serializer = PheedSerializer(page, many=True, context={'request': request})
...
def recommend_books(request):
    ...
    combined_terms = list(all_genres.union(all_keywords))[:10]
    recommended_books = []

    # 1. 네이버 먼저 호출
    try:
        recommended_books = search_books_on_naver(combined_terms, max_results=50)
    except Exception as e:
        print("⚠️ 네이버 API 실패 → 알라딘 fallback:", e)
        recommended_books = []

    # 2. 부족하면 알라딘으로 보완
    if len(recommended_books) < 50:
        try:
            needed = 50 - len(recommended_books)
            additional = search_books_on_aladin(combined_terms, max_results=needed)
            seen_isbns = {book['isbn'] for book in recommended_books}
            ...
            print("❌ 알라딘 API 실패 (fallback 채우기):", e)
